...
module Stationery =
    ...
    type Program(parent: HostWindow) =
        ...
        let canPrint state =
            not (String.IsNullOrWhiteSpace(state.TemplatePath)) && not (String.IsNullOrWhiteSpace(state.SourcePath))
        ...
        let update msg (state: State) =
            ...
            | CmdOpenFileDialogForSource ->
                state,
                Cmd.OfAsync.either openFileDialog (state.TemplatePath, parent) ChangeTemplatePath ShowErrorMessage
            | CmdOpenFileDialogForTemplate ->
                state,
                Cmd.OfAsync.either openFileDialog (state.SourcePath, parent) ChangeSourcePath ShowErrorMessage
